...
signal on_captions_toggled( value : bool)

signal window_mode_selected(index : int)
...
func emit_on_captions_toggled(value : bool) -> void:
	on_captions_toggled.emit(value)

func emit_window_mode_selected(index : int) -> void:
	window_mode_selected.emit(index)	

func emit_on_resolution_selected(index : int) -> void:
	resolution_selected.emit(index)
